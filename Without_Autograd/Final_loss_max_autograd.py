...
class CBFLoss:
    ...
    def diffs_fn(self, params, data_dict):
        ...
        return diffs
    def lip_fun_cal(self, params, data_dict ):
      ...
      permutation = jax.random.permutation(jax.random.PRNGKey(23), num_rows)
      data_all_selected_rows = data_all[permutation[:100], :]
      
      data_all_dists = data_dict['all_dists']
      num_rows = data_all_dists.shape[0]
      permutation = jax.random.permutation(jax.random.PRNGKey(23), num_rows)
      data_all_dists_selected_rows = data_all_dists[permutation[:100], :]
      
      
      data_all_inputs = data_dict['all_inputs']
      num_rows = data_all_inputs.shape[0]
      permutation = jax.random.permutation(jax.random.PRNGKey(23), num_rows)
      data_all_inputs_selected_rows = data_all_inputs[permutation[:100], :]
