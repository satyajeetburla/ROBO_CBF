...
def main(args):
    ...
    for step in range(args.n_epochs):
        ...
        if step % 100 == 0:
            ...
            with open(os.path.join(args.results_path, f'Total_loss_.txt'), 'w') as file:
                file.write(f'{loss:.3f}')
            #fname = os.path.join(args.results_path, 'lip_a.npy')
            #with open(fname, 'wb') as handle:
            #    pickle.dump(lip_a, handle, protocol=pickle.HIGHEST_PROTOCOL)
            #fname = os.path.join(args.results_path, 'lip_b.npy')
            #with open(fname, 'wb') as handle:
            #    pickle.dump(lip_b, handle, protocol=pickle.HIGHEST_PROTOCOL)
            #to load this use following command
            #fname = os.path.join(args.results_path, 'lip_a.npy')
