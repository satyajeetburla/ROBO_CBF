...
class CBFLoss:
    ...
    def loss_fn(self, params, data_dict):
        loss, _, _, _, _, _ = self.__loss(params, data_dict)
        return loss

    def constraints_fn(self, params, data_dict):
        _, constraints, _, _, _, _ = self.__loss(params, data_dict)
        return {key: float(value) for (key, value) in constraints.items()}

    def diffs_fn(self, params, data_dict):
        _, _, diffs, losses, _, _ = self.__loss(params, data_dict)

        return diffs

    def all_losses_fn(self, params, data_dict):
        _, _, _, losses, _, _ = self.__loss(params, data_dict)
        return {key: float(value) for (key, value) in losses.items()}

    def lip_acc_fun(self, params, data_dict):
        _, _, _, _, lip_a, lip_b = self.__loss(params, data_dict)
        return lip_a, lip_b
    ...
    def __loss(self, params, data_dict):
        ...
        return jnp.reshape(loss, ()), consts, diffs, losses, lip_const_a, lip_const_b
