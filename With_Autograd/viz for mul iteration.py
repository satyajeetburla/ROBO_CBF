...
class Visualizer:
    ...
    def state_separation(self, params, iteration):
        def make_df(safe):
    ...
    def level_sets(self, params, iteration):
        ...
        for d_idx in range(n_chunks):
            for v_idx in range(n_chunks):
                ...
                cte_range = np.linspace(min_cte, max_cte, num=200)
                theta_e_range = np.linspace(min_theta_e, max_theta_e, num=200)
                avg_d = (self._bins_d[d_idx + 1] + self._bins_d[d_idx]) / 2.0
                avg_speed = (self._bins_v[v_idx + 1] + self._bins_v[v_idx]) / 2.0
                ...
                name = f'{self._bins_d[d_idx]:.2f} ≤ d ≤ {self._bins_d[d_idx + 1]:.5f} and {self._bins_v[v_idx]:.5f} ≤ v ≤ {self._bins_v[v_idx + 1]:.2f}'
                ax.set_title(name)
                ax.set_xlabel('cte')
